...
public class PopIT {
	...
	public void discard(int choice, int stack1, int stack2)throws InvalidInputException{
		...
		switch(choice){
		case 1:if(stacks[stack1].peek().getRank().getRankNum().equals(stacks[stack2].peek().getRank().getRankNum()))
				{
					stacks[stack1].pop();
					stacks[stack2].pop();
					this.quantity -= 2;
		...
		case 2:if(stacks[stack1].peek().getSuit().equals(stacks[stack2].peek().getSuit()))
				{
					if(stacks[stack1].peek().getRank().getRankNum()
							< stacks[stack2].peek().getRank().getRankNum())
					{
						stacks[stack1].pop();
						this.quantity--;
					}
					else if(stacks[stack2].peek().getRank().getRankNum()
							< stacks[stack1].peek().getRank().getRankNum())
					{
						stacks[stack2].pop();
						this.quantity--;
					}
